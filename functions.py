...
def prime_num(num):
    if num < 2:
        return "The number is not prime"  # Edge case for numbers < 2

    for i in range(2, int(num ** 0.5) + 1):  # Check divisibility up to √num
        if num % i == 0:
            return "The number is not prime"
    
    return "The number is prime"  # If no divisors are found
...
describe_pet(petname="Moti")

# Return statement
def get_formatted_name(fn, ln):
    full_name = f"{fn} {ln}"
    return full_name
print(get_formatted_name('ram', 'lakhan'))
